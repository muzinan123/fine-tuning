...
def chat(user_input, chatbot, context, search_field, return_field):
    ...
    # Execute search when response starts with search command
    if "search:" in response:
        # Extract the latest JSON query condition after 'search:'
        ...
        if search_query is not None:
            ...
            # Send query results to LLM, let LLM generate a reply again
            response = get_completion(build_prompt(context))
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                ...
                user_input = gr.Textbox(show_label=False, placeholder="Input box...", lines=2)
                with gr.Row():
