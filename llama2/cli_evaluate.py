...
def parse_json(string):
    search_pos = 0
    # Start looking for the first '{'
    start = string.find('{', search_pos)
    if start == -1:
        return None
    # From the found '{' position, search backwards for the last '}'
...
class Evaluator:
    ...
    def compute_metrics(self):
        ...
        for data in tqdm(test_data):
            ...
            # Read context and generate prompt to input into the model
            ...
            # Parse the model output text and compare with label to calculate metrics
