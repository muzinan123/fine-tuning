...
def rrf(rankings, k=60):
    ...
    for ranking in rankings:
        if not ranking:  # If ranking is empty, skip it
            continue
...
class HotelDB():
          ...
          "classes": [
            {
              ...
              "properties": [
                ...
                },
                {
                  ...
                  "name": "_name", # Tokenized for search
                ...
                },
                {
                  ...
                  "name": "_address", # Tokenized for search
                  "indexSearchable": True,
                  "tokenization": "whitespace",
                  "moduleConfig": {
                    "text2vec-openai": { "skip": True }
    ...
    def search(self, dsl, name="Hotel", limit=1):
        # Filter out None values from dsl
        dsl = {k: v for k, v in dsl.items() if v is not None}
        _limit = limit + 10 # Search 10 more to get top `limit` items
        candidates = []
        ...
        if "facilities" in dsl:
            ...
            query = query.with_near_text(
                {"concepts": [f'Hotel provides:{";".join(dsl["facilities"])}']}
            )
            if filters:
